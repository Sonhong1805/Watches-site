...
if (paramsId) {
    ...
    const html = visibleProducts.map((product) => {
      ...
      <article class="product" data-detail=${product.id}>
        ...
        <div class="product__info">
          ...
          <button class="product__details-button"">Xem chi tiết</button>
        </div>
      </article>
    `;
    });
  ...
  const applyFilters = () => {
    ...
    const selectedCategory = [...categoryInputs].map(
  ...
  if (paramsQ) {
    document.querySelector(".header__bottom-search input").value = paramsQ;
    const searchArr = productList.filter((product) =>
      product.title.toLowerCase().includes(paramsQ.toLowerCase())
    );
